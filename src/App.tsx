...
async function loadSnipImage() {
  let lastError: unknown
  for (let i = 0; i < 4; i++) {
    try {
      const res = await fetch(`/api/snip?t=${Date.now()}`)
      if (!res.ok) {
  ...
  throw lastError instanceof Error ? lastError : new Error('Skrinshot olinmadi')
}
...
export default function App() {
  ...
  useEffect(() => {
    void fetch(`/api/armed?on=${active ? '1' : '0'}`)
  }, [active])
  ...
  useEffect(() => {
    ...
    source.onmessage = async (ev) => {
      ...
      try {
        const dataUrl = await loadSnipImage()
        ingestRef.current(dataUrl)
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Skrinshot olinmadi')
      } finally {
        ...
        <div className="command-main hud-frame">
          ...
          <div className="meta">
            ...
            <p className="muted small">
              {toggleLabel} bilan Start/Stop. Faol bo‘lgach {snipLabel} kesish oynasini ochadi,
              belgilangan joy tarjima qilinadi.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="stage">
        {current ? (
          ...
          <div className="empty hud-frame">
            <h2>Tarjima shu yerda chiqadi</h2>
            <p>
              <kbd>{toggleLabel}</kbd> → <kbd>{snipLabel}</kbd> → joyni belgilash.
            </p>
          </div>
        )}
      </section>
